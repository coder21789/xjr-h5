...
export default class AdCooperate extends Component {
  constructor(props) {
    super(props);
  ...
  handleClick() {
    setTimeout(() => {
      this.props.handleDialogShow();
    }, 500);
  };

	render() {
		const {handleDialogShow, theme} = this.props;
    return (
      <div className="fieldContainer">
        ...
        {theme < 2 ? (
          <div className="field">
            <div className="fieldMask under" onClick={this.handleClick}></div>
            <label>公司规模</label>
            <Control.text
              model=".companyScale"
            ...
            <Errors
              className="errors"
              model="user.companyScale"
              show={field => this.validator(field, 'companyScale')}
              messages={{
                required: '请选择您的公司规模'
              }}
            />  
